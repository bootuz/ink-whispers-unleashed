...
export function usePoems(page = 1) {
  return useQuery<PaginatedResponse<Poem>>({
    queryKey: ['poems', page],
    queryFn: () => fetchFromApi<PaginatedResponse<Poem>>(`${API_ENDPOINTS.poems}?page=${page}`),
  });
}
...
export function useAuthors(page = 1) {
  return useQuery<PaginatedResponse<Author>>({
    ...
    queryFn: () => fetchFromApi<PaginatedResponse<Author>>(`${API_ENDPOINTS.authors}?page=${page}`),
  });
}
